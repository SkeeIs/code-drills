...
function doubleLength(placeHolder){ 
    return str.length * 2; 
}
...
function arrayMerge(placeHolder) { 
    return arr.join(""); 
}
...
function arrSum(placeHolder) { 
	var nums = 0; 
    
    for (var i=0; i<placeHolder.length; i++) { 
		nums = nums + placeHolder[i]; 
	}
return nums; 
}
...
// 15, a = 9 from local assignment b = 6 from outside function
...
// 11 because the assigment of 9 to var a is never read by the function. <-- I was wrong. 
